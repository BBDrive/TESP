...
from meta_rl.policies.policy import Policy, weight_init
...
class NormalMLPPolicy(Policy):
    ...
    def __init__(self, input_size, output_size, hidden_sizes=(),
        ...
        self.sigma = nn.Parameter(torch.Tensor(output_size))  # 将方差变量变为可导的参数，不与神经网络连接
        self.sigma.data.fill_(math.log(init_std))
        self.apply(weight_init)

    def forward(self, observation, embedding, params=None):
        """

        :param input:
        :param params: 为None时即基于当前的actor network 参数
        :return:
        """
        size = list(observation.shape)
        size[-1] = len(embedding)
        embedding = embedding.expand(size)
        input = torch.cat((observation, embedding), dim=-1)
